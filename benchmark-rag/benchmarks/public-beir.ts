...
function prepareCorpus(corpus: BeirDoc[]): PreparedDoc[] {
  return corpus.map((doc) => {
    ...
    const allTokens = baseTokens(`${title} ${doc.text}`);
...
function productionCandidatePool(query: string, corpus: PreparedDoc[]) {
  ...
  return corpus
    .map((doc) => ({
      doc,
      overlap: terms.reduce((sum, term) => sum + (doc.bodyTokenSet.has(term) ? 1 : 0), 0),
    }))
...
function buildBm25Stats(corpus: PreparedDoc[]) {
  ...
  return {
    documentFrequency,
    averageLength: totalLength / Math.max(corpus.length, 1),
  };
}
...
function idealDcg(relevant: Map<string, number>, k: number) {
  const grades = [...relevant.values()].sort((a, b) => b - a).slice(0, k);
  return grades.reduce((sum, relevance, index) => sum + (2 ** relevance - 1) / Math.log2(index + 2), 0);
}
...
function evaluate(run: Run, qrels: Qrels) {
  ...
  return {
    queries: evaluated,
    hitAt1: divide(hit1),
    recallAt3: divide(recall3),
    recallAt10: divide(recall10),
    recallAt20: divide(recall20),
    mrrAt10: divide(mrr10),
    ndcgAt10: divide(ndcg10),
    mapAt10: divide(map10),
    queryCoverage: divide(returnedAny),
  };
}
...
const productionRun: Run = new Map();
const bm25Run: Run = new Map();
...
const report = {
  ...
  dataset: datasetName,
  datasetPath: path.resolve(datasetDir),
  ...
  productionConfiguration: {
    candidateLimit,
    resultLimit,
    candidateSelection: "plaintext equality-equivalent of production blind-index overlap prefilter; stable document-id tie-break added for reproducibility",
    rankingModule: "src/lib/ranking.ts",
    externalModel: false,
    externalApi: false,
    embeddings: false,
  },
  systems: {
    deterministicProductionRetriever: {
      metrics: evaluate(productionRun, qrels),
      latencyMs: { p50: percentile(productionTimes, 0.5), p95: percentile(productionTimes, 0.95) },
    },
    localBm25Baseline: {
      description: "Plain BM25 over title + body using the same base tokenizer; not an official Pyserini/BEIR leaderboard run.",
      metrics: evaluate(bm25Run, qrels),
      latencyMs: { p50: percentile(bm25Times, 0.5), p95: percentile(bm25Times, 0.95) },
    },
  },
};
